...
func HandleListModelMappings(c *gin.Context) {
	...
	mappings, err := ProviderService.ListModelMappings()
	if err != nil {
		Logger.WithError(err).Error("Failed to list model mappings")
		c.JSON(http.StatusInternalServerError, gin.H{
...
func HandleCreateModelMapping(c *gin.Context) {
	...
	if err := ProviderService.CreateModelMapping(&mapping); err != nil {
		Logger.WithError(err).Error("Failed to create model mapping")
		c.JSON(http.StatusInternalServerError, gin.H{
...
func HandleDeleteModelMapping(c *gin.Context) {
	...
	id := c.Param("id")
	if err := ProviderService.DeleteModelMapping(id); err != nil {
		Logger.WithError(err).Error("Failed to delete model mapping")
		c.JSON(http.StatusInternalServerError, gin.H{
...
func HandleImportConfig(c *gin.Context) {
	...
	if err := BackupService.RestoreBackup(req.BackupPath, req.Restore); err != nil {
